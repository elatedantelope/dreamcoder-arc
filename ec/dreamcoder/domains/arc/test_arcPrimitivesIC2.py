...
#Grid = NewType("Grid", Tuple[Tuple[int]])
...
Integer = int
tcord = baseType("cord")
Cord = NewType("Cord", Tuple[int, int])
...
def gridToTT(x:Grid) -> dslGrid:
    grid = x.grid
    #turn this np.ndarray to tuple[tuple[int]]
...
def objectS(
    ...
    bg = mostcolordslGrid(grid) if without_bg else None
    objs = set()
    occupied = set()
...
def partition(
    ...
    return frozenset(
        ...
        ) for value in palettedslGrid(grid)
    )
...
def fgpartition(
    ...
    return frozenset(
        frozenset(
            (v, (i, j)) for i, r in enumerate(grid) for j, v in enumerate(r) if v == value
        ) for value in palettedslGrid(grid) - {mostcolordslGrid(grid)}
    )
...
def squaredslGrid(
    ...
    return len(piece) == len(piece[0]) if isinstance(piece, tuple) else heightdslGrid(piece) * widthdslGrid(piece) == len(piece) and heightdslGrid(piece) == widthdslGrid(piece)
...
def numcolorsdslGrid(
    ...
    """ number of colors occurring in object or grid """
    return len(palettedslGrid(element))
...
def frontiers(
    ...
    column_indices = tuple(j for j, c in enumerate(dmirrordslGrid(grid)) if len(set(c)) == 1)
...
def compress(
    ...
    ci = tuple(j for j, c in enumerate(dmirrordslGrid(grid)) if len(set(c)) == 1)
